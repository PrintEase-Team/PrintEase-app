...
import com.group108.printease.service.PdfService;
...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentServiceImpl(PaymentRepository paymentRepository, OrdersRepository ordersRepository, PrintSettingsRepository printSettingsRepository, NotificationsService notificationsService, FilesRepository filesRepository, PdfService pdfService) {
        ...
        try {
            ...
            if (body != null && Boolean.TRUE.equals(body.get("status"))) {
                ...
                if ("success".equals(data.get("status"))) {
                    ...
                    // Slice PDF if custom page range
                    List<Print_Settings> settingsList = printSettingsRepository.findByOrder(order);
                    List<Files> orderFiles = filesRepository.findByOrder(order);
                    if (!settingsList.isEmpty() && !orderFiles.isEmpty()) {
                        Print_Settings settings = settingsList.get(0);
                        Files file = orderFiles.get(0);
                        if (settings.getPage_range() != null && !settings.getPage_range().trim().isEmpty() && !settings.getPage_range().equalsIgnoreCase("All")) {
                            int newPageCount = pdfService.slicePdf(file.getStorage_url(), settings.getPage_range());
                            if (newPageCount > 0) {
                                file.setPage_count(newPageCount);
            ...
            throw new RuntimeException("Error verifying payment with Paystack: " + e.getMessage());
        }
    }
}
